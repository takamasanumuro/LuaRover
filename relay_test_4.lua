...
function update()
    ...
    local throttle_left_channel = SRV_Channels:find_channel(throttle_left_function)
    local throttle_right_channel = SRV_Channels:find_channel(throttle_right_function)

    local timeout = 20

    if throttle_right_pwm > 1500 then
        local throttle_right_new_pwm_high = uint32_t(map(throttle_right_pwm, 1500, 2000, 1800, 2000))
        SRV_Channels:set_output_pwm_chan_timeout(throttle_right_channel, throttle_right_new_pwm_high:toint(), timeout)
        ...
        SRV_Channels:set_output_pwm_chan_timeout(throttle_right_channel, throttle_right_new_pwm_low:toint(), timeout)
    ...
    if throttle_left_pwm > 1500 then
        ...
        SRV_Channels:set_output_pwm_chan_timeout(throttle_left_channel, throttle_left_new_pwm_high:toint(), timeout)
        ...
        SRV_Channels:set_output_pwm_chan_timeout(throttle_left_channel, throttle_left_new_pwm_low:toint(), timeout)
